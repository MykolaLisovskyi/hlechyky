...
- `Act` для покрокових ігор: кидає `GameError("Зараз не твій хід")` або повертає `ActResult.Fail(...)` —
  обидва доходять до гравця як тост, нікому більше. Хід прийнято — `ActResult.Done`; прийнято, і є що
  сказати тому, хто ходив, — `ActResult.Accept("Запропонував нічию")`. Після обох каркас розішле нові види.
...
Ставки є лише в іграх на двох із рейтингом (`MaxPlayers == 2 && Rated`) — настільні, змійка, дуель-вестерн;
у Party/Solo і в нерейтингових їх нема. Ставка, обрана в непідходящій грі або поза списком, мовчки стає 0.
...
Так живуть `scrabble-30`, `mafia-win`, `sheriff`, `svoya-win`. Ачівки, які видно з подій платформи (`duel-fast` —
...
- Глобальний `HGames` (єдиний об'єкт у `window`). `app.js` дає йому `conn`, `me`, `toast`, `busy`, `esc`,
  `api`, контейнер `#games`, функцію переходу `go(hash)`, і кличе `HGames.attach(conn)` у `connect()`,
  `HGames.reconnected()` після `conn.start()` і після реконекту, `HGames.show(tail)` / `HGames.hide()`
  при зміні маршруту. Більше `app.js` про ігри не знає.
...
- **Маршрути.** `tail` у `show(tail)` — це те, що в адресі після `#games/`: порожньо — лобі,
  ...
  приходить `show(tail)`. Тому F5 і посилання на стіл працюють без окремого коду.
- **Лобі** (`#games`): зверху «🔥 Живі столи» — **усі** відкриті кімнати з усіх груп, свої першими, з
  ніками, вільними місцями, станом партії і кнопками «Сісти» / «Дивитись» / «Відкрити». Нижче «Каталог»:
  усі ігри однією сіткою плиток із чипами-фільтрами (Усі · Настільні · Швидкі · Компанія · Соло, стан у
  `localStorage('gamesFilter')`) і полем «знайти гру». Групи більше не вкладки, тож стіл видно з будь-якого
  фільтра. Лічильник живих столів `core` малює в `#navGames` у шапці сайту, а гаманець — у `#hdrWallet`.
- **Резюме столу в лобі — окремий елемент** (`.gsum`), а НЕ копія картки: повна картка з модулем гри живе
  лише на сторінці столу.
- **Сторінка столу** (`#games/room/<id>`): рядок «← Лобі · іконка й назва · 👁 · чипи інших моїх столів ·
  ⛶» і картка кімнати на всю ширину змісту (до 1600px). `⛶` згортає балачки й підрозділи (клас
  `body.gfull`); `Esc` спершу знімає `⛶`, потім вертає в лобі.
- **Картка кімнати створюється рівно раз** і далі лише переїжджає між складом (`.gtables`, завжди
  прихований) і сторінкою столу (`.grbox`), а невидимі ховаються через `hidden`. Перестворити її означало
  б відібрати в модуля канвас, таймери й половину стану, тому `ensureCard(id, host)` вставляє картку в DOM
  **до** першого `mount()` — модуль, який міряє ширину, має що міряти. Картка живе, поки живе кімната.
- Картка (спільна для всіх ігор): шапка (іконка, назва, чіпи місць із ніками, чия черга/рахунок),
  тіло (модуль гри), рядок статусу, кнопки (Сісти / Встати / Почати / Ще раз / Дивлюсь збоку). Модуль
  малює **лише** тіло.
- **Підписки на кадри** (`WatchRoom`): відкритий стіл, щойно відкриті приватні кімнати і ті, де я сиджу
  (щоб у лобі було видно «твій хід»). Решта — ні: реалтайм-кімната сипле десятки повідомлень на секунду.
  Перші `rooms` прилітають ще до того, як `conn.start()` віддасть `Connected`, тому `app.js` кличе
  `HGames.reconnected()` одразу після старту — інакше стіл, відкритий за посиланням, вічно «завантажувався».
...
- Клавіатура: `core` тримає «активну кімнату» — це **видима** картка (де я сиджу і йде партія; інакше
  перша видима), і віддає їй `keydown`. Картки на складі клавіш не отримують. Розділи сайту (`1/2/3`,
  `]`, `Esc`) слухають `document` після `core`, тому клавішу, яку гра вже з'їла (`preventDefault`),
  не перехоплюють. Для телефона модуль сам малює свої кнопки (`HGames.ui.dpad`, свої).
- Мобільний: чотири вкладки внизу, над ними міні-плеєр (у режимі `⛶` ховається). Каталог — компактний
  список, живі столи — одна колонка, рядок столу тримається в один рядок.
- Стилі: `core.css` (перенесений блок `.games/.gtable/...` зі `style.css`) + `<id>.css` на гру. Кольори —
  тільки через змінні `--accent/--ok/--clay/--bg2/...`. Секції лобі, підрозділи й сама картка столу —
  такі самі панелі, як у решті сайту (`.gpanel`, `.gview.boxed`), тож `#colGames` уже не панель.
  Стелі ширини: `.gcanvas` — 560px, `.board` — 360px (`.board.discs` — 460px); гра, якій треба більше,
  піднімає свою стелю у власному css (шахи, шашки, ерудит так і роблять).
